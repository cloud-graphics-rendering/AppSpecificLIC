...
class NetXSmall(nn.Module):
  def __init__(self):
    ...
    self.h2_sigma = nn.Parameter(torch.ones((1,64*4,1,1), dtype=torch.float32, requires_grad=False))
  ...
  def decode(self, inputs, stage):
    if stage == 0:
      ...
      pf = self.s_model(z3_rounded)
      x_tilde = self.side_recon_model(pf, self.h2)
...
def compress_low(args, input_image, net):
  ...
  for block in blocks:
    with torch.no_grad():
      ...
      sigma_val = np.reshape(v_z2_sigma, (1,-1,1,1))
...
def decompress_low(args, net):
  ...
  if pad_down != 0 and pad_right != 0:
    make_img(tar[0, :, pad_up:-pad_down,pad_left:-pad_right], args.outputpng)
  elif pad_down != 0:
    make_img(tar[0, :, pad_up:-pad_down,pad_left:], args.outputpng)
  elif pad_right != 0:
    make_img(tar[0, :, pad_up:,pad_left:-pad_right], args.outputpng)
  else:
    make_img(tar[0, :, pad_up:,pad_left:], args.outputpng)
